...
int main(int argc, char* argv[])
{
    ...
    {
        colors[i] = XY_setcolor(128 + (rand() % 128), 128 + (rand() % 128),
                                128 + (rand() % 128), 0xff);
    }
    ...
    do
    {
        ...
        for (i = 0; i < NUM_ROCKS; i++)
        {
            ...
            x2 = rocks[i].x +
                 XY_mult(XY_cos((rocks[i].angle >> XY_FIXED_SHIFT) + 120),
                         RADIUS);
            y2 = rocks[i].y -
                 XY_mult(XY_sin((rocks[i].angle >> XY_FIXED_SHIFT) + 120),
                         RADIUS);

            x3 = rocks[i].x +
                 XY_mult(XY_cos((rocks[i].angle >> XY_FIXED_SHIFT) + 240),
                         RADIUS);
            y3 = rocks[i].y -
                 XY_mult(XY_sin((rocks[i].angle >> XY_FIXED_SHIFT) + 240),
                         RADIUS);
